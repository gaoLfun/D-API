...
[简体中文](README.md) · [Documentation index](docs/README.md) · [Deployment](docs/deployment.en.md)

**A lightweight AI API gateway for NewAPI and Sub2API with multi-upstream failover,
group-scoped API keys, priority routing, and an operations dashboard.**

![D-API dashboard (example data)](docs/screenshots/dashboard-light.png)
...
This README describes the current `main` branch. The project is usable, but its
configuration and management API may still change before v1.0. See the
[changelog](CHANGELOG.md) for released-version capabilities.
...
![D-API routing topology (example data)](docs/screenshots/dashboard-topology-light.png)

## Good Fit

- A self-hosted NewAPI or Sub2API gateway with priority-based failover
- Small teams that need group-scoped client keys and model/protocol restrictions
- OpenAI- or Anthropic-compatible clients that should keep their native protocol
- Operators who need balance protection, cache metrics, usage analytics, and
  estimated cost

## Not a Fit

D-API does not provide multi-tenant organizations, reseller billing, payment
processing, provider-account login, protocol translation, or distributed
...
[Deployment](docs/deployment.en.md) for temporary direct-IP HTTP access.

Log in with `DAPI_ADMIN_USERNAME` and `DAPI_ADMIN_PASSWORD`, then:

1. Add a NewAPI or Sub2API upstream and use **Fetch upstream models** before
   saving.
2. Select models and run **Test model** for one or more entries. This sends a
   small real request and may consume upstream quota.
3. Create a group and bind upstreams; requests stay within the key's group.
4. Optionally assign a pricing profile for estimated cost reporting.
5. Select supported protocols and models. Lower priority numbers route first.
6. Create a client API key and select its group. The list shows only its prefix;
   use the copy button or CCSwitch import. Old unrecoverable keys must be
   recreated.

For production details, direct-IP HTTP access, upgrades, backups, and restores,
see [Deployment](docs/deployment.en.md). For a scriptable administrator interface,
...
protocols. See [API compatibility](docs/api-compatibility.en.md) for authentication,
...
See [Architecture](docs/architecture.en.md) for routing, health, persistence, and
security boundaries.

## FAQ

**Does upstream priority still apply after a client key is bound to a group?**

Yes. D-API first selects the key's group, then tries only that group's upstream
keys in ascending numeric priority. Grouping identical Base URLs in the UI does
not change key-level routing.

**Is the estimated official cost the same as the provider invoice?**

No. It is an operational estimate based on recorded token usage and the
LiteLLM/manual pricing profile effective for the request. Missing prices remain
unknown and are represented by cost coverage.

**What happens when an upstream runs out of balance?**

With balance protection enabled, two consecutive automatic zero-balance checks
(or one manual check) suspend the upstream. A positive or unlimited result
...
- [Deployment, backup, and restore](docs/deployment.en.md)
- [API compatibility](docs/api-compatibility.en.md)
...
D-API is intentionally dedicated and small. Before v1.0 it does **not** aim to provide:
