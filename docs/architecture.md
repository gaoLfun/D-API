# 架构与路由

[English](architecture.en.md)

本文描述 D-API 当前 `main` 分支的实现，而非未来路线图或稳定性承诺。

## 组件

```text
客户端 ---- HTTPS ---- Caddy ----> D-API Go 进程 ----> NewAPI / Sub2API 上游
管理员 ---- HTTPS ---------------->  |  API 网关
                                     |  管理 API 与 Vue SPA
                                     |  健康/余额/价格/告警任务
                                     +------ PostgreSQL
```

- **Caddy**：在 Compose 部署中终止 TLS 并反向代理到 D-API。
- **D-API**：同一个 Go 进程提供客户端 API、管理 API、后台静态文件和后台任务。
- **PostgreSQL**：保存配置、加密凭据、会话、健康/熔断状态、请求元数据、用量、审计和告警。
- **Vue SPA**：构建后由 Go 进程同源提供。

当前正式支持单实例部署。并发计数、登录限流和部分通知冷却状态位于进程内，
因此多副本不是当前承诺的运行拓扑。

## 请求路由流程

1. 对客户端提交的 `dapi_` Key 做哈希认证，并确认 Key 已启用。
2. 校验 Key 允许的协议、模型和所属分组。
3. 只加载该分组内已启用的上游，按数字优先级从小到大排序。
4. 排除已停用、余额暂停、熔断中、协议不兼容或模型不兼容的候选。
5. 在 `max_attempts` 范围内依次尝试；默认 3，允许范围 1～5。
6. 如果配置模型别名，只重写 JSON 顶层的 `model` 字段。
7. 返回响应，并记录请求 ID、尝试链、状态、延迟和可获得的 Token 用量。

客户端 Key、路由候选和 `max_attempts` 使用短时进程内缓存；通过管理 API 修改相关资源后会
立即清除对应缓存。客户端 Key 和路由缓存分别最多保留 4096 和 1024 项，路由缓存最多滞后
2 秒，因此仍以 PostgreSQL 为配置事实来源。

同键缓存未命中时合并加载；无路由变化的探测不再清空缓存。价格按请求开始时间从
有界历史版本缓存中选择，总览结果允许最多 3 秒缓存延迟，统计口径保持不变。
实现边界和分配量基准见 [性能说明](performance.md)。

原始请求体复用前会拒绝顶层重复或大小写不规范的 `model` / `stream` 字段，确保网关
和上游对权限相关字段采用同一解释。价格历史超过 256 条时切换为有效时间区间缓存。

相同优先级按数据库顺序处理。当前不实现权重、随机、成本优先或最低延迟负载均衡。

## 分组、优先级与 Base URL 聚合

客户端 Key 必须绑定一个分组，分组绑定一个或多个上游。优先级始终属于具体上游 Key：
请求先进入 Key 的分组，再在该分组成员中按 Key 优先级路由，不会回退到其他分组或
全局池。

管理界面会把规范化后相同 Base URL 的多个 Key 聚合成一个上游集群，方便查看同一服务
下的多账号状态。该聚合只影响界面与集群数量统计：

- 路由仍按每个 Key 的优先级、健康、模型和协议判断。
- 余额、账号扣费和官方价格估算不会在聚合行相加，而是在抽屉中分别展示。
- 拓扑视图是运维摘要，不改变实际 SQL 候选筛选规则。

## 故障切换与熔断

D-API 在以下情况尝试下一个候选上游：

- 连接、首包或响应空闲超时；
- 首字节发出前的传输、读取错误；
- HTTP 401、403、404、429 或 5xx。

其他 HTTP 状态直接返回。401/403 会立即打开熔断；普通失败累积到阈值后进入不健康状态，
冷却结束后重新允许尝试。成功请求会清除连续失败计数。

流式响应只有在首字节发出前才能安全切换上游；一旦响应已提交，中途断流只能记录失败，
不能重放或拼接流。

## 后台任务

- **健康检查**：默认每 30 秒请求上游 `/v1/models`，并在模型列表未锁定时更新模型。
- **余额检查**：默认每 10 分钟探测兼容的 NewAPI/Sub2API 余额接口；余额暂停但仍启用的上游
  会继续探测，以便余额恢复后清除暂停状态。手动停用的上游不执行定时探测。
- **余额保护**：自动检查连续两次为零（手动刷新一次）后暂停，恢复为正数或无限额度后恢复。
- **价格同步**：每天从 LiteLLM 结构化价格文件更新受管价格档案；自定义档案不被覆盖。
- **告警评估**：默认每分钟评估规则，将邮件或 Webhook 通知写入 PostgreSQL outbox。
  错误率和延迟至少需要 5 次请求尝试，连续 2 次异常才触发，连续 3 次低于恢复阈值才恢复；
  恢复阈值分别为触发阈值的 75% 和 80%。样本不足或评估中断不会当作恢复。
  上游事件短暂复发沿用冷却和提醒额度，连续正常 30 分钟才重置；状态及确认进度持久化。
- **通知适配**：Webhook 支持钉钉、飞书/Lark、企业微信、Slack、Discord 和通用 JSON；
  可显式指定平台，也可按受支持的 URL 域名自动识别。告警持续期间按规则上限抑制重复
  通知，恢复事件仍会发送。outbox worker 默认等待 10 秒聚合、每 5 秒轮询，单批最多
  50 条，按事件类型、状态、级别和通知渠道合并；每个渠道投递失败按 10 秒、1 分钟、
  5 分钟退避，最多尝试 5 次，超过上限保留为死信。
- **健康告警降噪**：降级状态及其恢复只记录到数据库，不推送通知；异常状态只发送一次，
  定时或手动健康探测连续成功 3 次，或恢复过程持续 2 分钟后才确认恢复并发送一次恢复通知，
  期间再次失败会清零恢复计数。实时代理请求成功不会推进恢复确认。
  通知独立使用持久化事件状态，短暂复发在 30 分钟冷却内合并，最多提醒 3 次，
  连续正常 30 分钟后重置额度；路由隔离不等待通知冷却。
- **数据清理**：启动时和每小时按独立保留期清理日志、日/小时汇总；同 schema 互斥，每表 15 秒预算，每批最多 1000 条，批间退让 25ms，指标记录删除量和未完成状态。

健康和余额任务独立调度，每类最多 8 个并发探测，手动探测也受同类上限约束。
余额查询记住成功的非订阅接口路径一小时，路径失效或配置变化后重新发现；订阅接口仍仅作为兜底，
缓存只保存路径，不缓存余额结果。健康检查确认模型列表接口可用，不代表每个模型都能完成推理。

## 用量、缓存与成本

D-API 保存请求结果、总耗时、TTFB、可观测的流式 TTFT、尝试链、客户端 IP 和上游返回的
Token 用量；不保存请求体或响应体。

生产网关将请求记录放入最多 2048 条的有界队列，每 100 毫秒或累计 64 条后批量提交。
同批的日、小时和上游生命周期统计会先按维度合并，再更新 PostgreSQL。队列已满时请求会
同步写入以施加背压；批量写入失败会按有界退避重试，最终失败会累计丢弃计数并由正常关闭
返回错误。正常关闭会刷新队列；若超过关闭期限，则取消正在进行的写入、将剩余记录计入
丢弃数量，并在数据库关闭前停止记录协程。进程被强制终止时，尚未刷新的少量记录仍可能丢失。

停机先停止接收新请求，最多等待 10 秒；随后取消未完成请求并关闭连接，最多再等 5 秒
让处理器提交日志，最后给予日志刷新最多 10 秒。Compose 停止宽限期为 35 秒。
任一收尾阶段超时或日志刷新失败都会返回停机错误；超过预算仍未提交的记录无法保证保存。

用量可以按上游、客户端 Key、分组、协议或模型聚合，支持平均/P95 延迟、缓存读写、
Token 命中率和请求命中率。成本由请求发生时匹配的价格档案计算：

- 受管档案来自 LiteLLM，手动档案作为兜底；
- 未知价格或缺失 Token 的请求不猜测成本，并计入覆盖率；
- 回算只补齐未知成本，不覆盖已确定历史金额；
- `upstream_lifetime_usage` 独立保存按上游 Key 的生命周期请求与估算成本，不受普通日志
  和日用量清理影响。

这些数值是运营估算，不是供应商账单或客户端扣费。

## 数据与安全边界

- 上游 API Key、可选余额凭据、通知配置和客户端 Key 可恢复副本使用 AES-256-GCM 加密。
- 客户端 Key 认证使用 SHA-256 哈希；管理会话使用独立的 HttpOnly Cookie。
- 管理员密码使用 bcrypt；修改或重置密码会撤销已有会话。
- 管理修改请求校验 Origin，登录按客户端 IP 和账号标识限速。
- 出站 HTTP/SMTP 目标会拒绝回环、私网、链路本地、组播、CGNAT 和云元数据地址；
  连接前会重新解析并校验，且不跟随重定向。
- 余额查询失败时会保留上次成功的 `used`、币种和成功时间，同时单独标记当前探测失败。
- 告警只在观测结果明确时改变状态；余额未知或不可用会忽略本次观测，不会误触发恢复。
- `DAPI_MASTER_KEY` 不写入 PostgreSQL；数据库备份必须与同一主密钥配套保管。

## 持久化与迁移

数据库结构嵌入二进制并在启动时通过带版本记录的事务迁移更新；已应用版本不再执行。当前没有独立迁移命令和自动回滚，
PostgreSQL 是唯一支持的数据库。删除活动上游或客户端 Key 后，会尽可能保留历史用量标识。

## 默认运行限制

- 客户端请求体：32 MiB。
- 非流式上游响应缓冲：32 MiB。
- 健康/余额探测响应：1 MiB。
- 全局并发：256；每客户端 Key 并发：32。
- 每 Key 每分钟请求：600。
- 单请求最长生命周期：15 分钟。

环境变量和完整限制见[配置文档](configuration.md)。
