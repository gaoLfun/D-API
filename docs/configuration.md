...
| `DAPI_ADMIN_PASSWORD` | Required on first start; minimum 12 characters | Initial administrator password |
...
| `DAPI_TRUST_PROXY` | `false` in the binary; `true` in Compose | Trust a valid `X-Real-IP` supplied by the immediate proxy |
