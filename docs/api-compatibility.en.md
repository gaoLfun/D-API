...
[中文](api-compatibility.md)

D-API exposes a small pass-through subset of OpenAI- and
...
- `/v1/models` applies the key's model restriction and only lists models from
  its group.
...
entries use `owned_by: "dapi"` and `created: 0`; D-API does not proxy this
call to a single upstream.
