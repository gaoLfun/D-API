...
test('capture the operational surface across themes and viewports', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: '删除上游' }).click()
  ...
  await page.getByTitle('展开详情').first().click()
...
test('dialogs preserve keyboard focus and close predictably', async ({ page }) => {
  ...
  const menuTrigger = page.getByTitle('更多操作').first()
  await menuTrigger.click()
  await expect(page.getByRole('menuitem', { name: '刷新余额' })).toBeFocused()
  await page.keyboard.press('ArrowDown')
  await expect(page.getByRole('menuitem', { name: '刷新模型' })).toBeFocused()
  await page.keyboard.press('Escape')
...
test('direct routes, chained dialogs, dates, and edge menus remain operable', async ({ page }) => {
  ...
  const menuTrigger = page.getByTitle('更多操作').last()
  await menuTrigger.scrollIntoViewIfNeeded()
  await menuTrigger.click()
  expect(await page.getByRole('menu').evaluate((element) => element.getBoundingClientRect().bottom <= window.innerHeight - 8)).toBe(true)
})
